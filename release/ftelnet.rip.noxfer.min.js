...
else if(this._NegotiationState===RLoginNegotiationState.SS){if(++this._SSBytes>=8){this._SSBytes=0;this._NegotiationState=RLoginNegotiationState.Data;}}}};return RLoginConnection;}(WebSocketConnection));var RLoginNegotiationState;(function(RLoginNegotiationState){RLoginNegotiationState[RLoginNegotiationState["Data"]=0]="Data";RLoginNegotiationState[RLoginNegotiationState["Cookie1"]=1]="Cookie1";RLoginNegotiationState[RLoginNegotiationState["Cookie2"]=2]="Cookie2";RLoginNegotiationState[RLoginNegotiationState["S1"]=3]="S1";RLoginNegotiationState[RLoginNegotiationState["SS"]=4]="SS";})(RLoginNegotiationState||(RLoginNegotiationState={}));var io=window.io;var SocketIoConnection=(function(_super){__extends(SocketIoConnection,_super);function SocketIoConnection(){var _this=_super.call(this)||this;_this.SocketIOConnected=false;_this.handleSocketIOConnect=function(){_this.SocketIOConnected=true;_this.onconnect.trigger();_this.SocketIO.emit("geometry",80,25);_this.SocketIO.emit("terminal","ansi");};_this.handleSocketIODisconnect=function(){if(_this.SocketIOConnected){_this.onclose.trigger();}
_this.SocketIOConnected=false;};_this.handleSocketIOData=function(data){var bytes=[];for(var i_1=0;i_1<data.length;++i_1){var code=data.charCodeAt(i_1);bytes=bytes.concat([code]);}
data=bytes;if(_this._InputBuffer.bytesAvailable===0){_this._InputBuffer.clear();}
var OldPosition=_this._InputBuffer.position;_this._InputBuffer.position=_this._InputBuffer.length;var Data=new ByteArray();var i;for(i=0;i<data.length;i++){Data.writeByte(data[i]);}
Data.position=0;_this.NegotiateInbound(Data);_this._InputBuffer.position=OldPosition;_this.ondata.trigger();};return _this;}
Object.defineProperty(SocketIoConnection.prototype,"bytesAvailable",{get:function(){return this._InputBuffer.bytesAvailable;},enumerable:true,configurable:true});SocketIoConnection.prototype.close=function(){if(this.SocketIOConnected){this.SocketIO.close();}};SocketIoConnection.prototype.connect=function(hostname,port,urlPath,forceWss,proxyHostname,proxyPort,proxyPortSecure){if(!io){throw"socket.io-client not found";}
var host=(forceWss?'wss':'ws')+"://"+hostname+":"+port+"/";this.SocketIO=io(host);this.SocketIO.on("connect",this.handleSocketIOConnect);this.SocketIO.on("disconnect",this.handleSocketIODisconnect);this.SocketIO.on("data",this.handleSocketIOData);};Object.defineProperty(SocketIoConnection.prototype,"connected",{get:function(){return this.SocketIOConnected;},enumerable:true,configurable:true});SocketIoConnection.prototype.Send=function(data){this.SocketIO.emit('data',data);};return SocketIoConnection;}(WebSocketConnection));var TelnetCommand;(function(TelnetCommand){TelnetCommand[TelnetCommand["EndSubnegotiation"]=240]="EndSubnegotiation";TelnetCommand[TelnetCommand["NoOperation"]=241]="NoOperation";TelnetCommand[TelnetCommand["DataMark"]=242]="DataMark";TelnetCommand[TelnetCommand["Break"]=243]="Break";TelnetCommand[TelnetCommand["InterruptProcess"]=244]="InterruptProcess";TelnetCommand[TelnetCommand["AbortOutput"]=245]="AbortOutput";TelnetCommand[TelnetCommand["AreYouThere"]=246]="AreYouThere";TelnetCommand[TelnetCommand["EraseCharacter"]=247]="EraseCharacter";TelnetCommand[TelnetCommand["EraseLine"]=248]="EraseLine";TelnetCommand[TelnetCommand["GoAhead"]=249]="GoAhead";TelnetCommand[TelnetCommand["Subnegotiation"]=250]="Subnegotiation";TelnetCommand[TelnetCommand["Will"]=251]="Will";TelnetCommand[TelnetCommand["Wont"]=252]="Wont";TelnetCommand[TelnetCommand["Do"]=253]="Do";TelnetCommand[TelnetCommand["Dont"]=254]="Dont";TelnetCommand[TelnetCommand["IAC"]=255]="IAC";})(TelnetCommand||(TelnetCommand={}));var TelnetConnection=(function(_super){__extends(TelnetConnection,_super);function TelnetConnection(crt){var _this=_super.call(this)||this;_this._Crt=crt;_this._NegotiatedOptions=[];for(var i=0;i<256;i++){_this._NegotiatedOptions[i]=0;}
...
switch(this._Options.ConnectionType){case'rlogin':this._Connection=new RLoginConnection();break;case'tcp':this._Connection=new WebSocketConnection();break;case'socketio':this._Connection=new SocketIoConnection();break;default:this._Connection=new TelnetConnection(this._Crt);this._Connection.LocalEcho=this._Options.LocalEcho;this._Connection.onlocalecho.on(function(value){_this.OnConnectionLocalEcho(value);});this._Connection.SendLocation=this._Options.SendLocation;break;}
